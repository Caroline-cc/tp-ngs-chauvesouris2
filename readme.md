...
Before running blast, we had to build a reference database listing human CDS for subsequent alignment. Human CDS were imported from Ensembl online database with the wget command. Then makeblastdb application enabled to organize those CDS in a database assigning an unique identifier to every sequence. Nucleotidic nature of the downloaded sequences and their fasta format were indicated respectively with dbtype 'nucl'and -parse_seqids parameters.
...
Salmon provides probabilistic values of the effective length of transcripts,  
abundance and raw counts. The effective length is a length estimation taking into account biases like gc-bias and the fragment length distribution. Num reads are an estimation of the number of reads derived from each transcript since some reads are multi-mapping. The transcripts per million (TPM) results are the ones used for quantification. They are an estimation of the relative abundance of  transcripts and include a normalization by gene length and sequencing depth. 
...
Nevertheless, considering the few number of replicates per conditon (3), we chose to conserve the <sup> rd</sup> sample in downstream analysis, for a better statistical power.
